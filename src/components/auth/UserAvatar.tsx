...
function getInitials(nameOrEmail: string): string {
  ...
  const parts = namePart
    ...
    .split(' ')
    .filter(Boolean);
  const letters = parts.length >= 2 ? parts[0][0] + parts[1][0] : (parts[0]?.slice(0, 2) ?? 'U');
...
export function UserAvatar({ user, className }: { user: User; className?: string }) {
  ...
  const imageUrl =
    ...
    (typeof (meta as Record<string, unknown>).picture === 'string' &&
      ((meta as Record<string, unknown>).picture as string)) ||
    undefined;
  const label = (typeof meta.name === 'string' && meta.name) || user.email || 'User';
