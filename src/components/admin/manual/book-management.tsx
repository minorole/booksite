...
export function BookManagement() {
  ...
  const fetchBooks = async () => {
    try {
      ...
      // Transform the books to match BookData type
      const transformedBooks: BookData[] = data.books.map((book: any) => ({
        ...book,
        ...
        discontinued_at: book.discontinued_at || null,
        discontinued_by: book.discontinued_by || null,
        discontinued_reason: book.discontinued_reason || null,
        last_quantity_update: book.last_quantity_update || null
      }));
  ...
  const handleSave = async (updatedBook: BookData) => {
    try {
      const response = await fetch(`/api/admin/books/${updatedBook.id}`, {
        ...
        body: JSON.stringify({
          ...
          discontinued: updatedBook.discontinued,
          discontinued_reason: updatedBook.discontinued_reason
        }),
      });
      ...
      // Transform the response to match BookData type
      const transformedBook: BookData = {
        ...data.book,
        auto_tags: data.book.auto_tags || [],
        pending_tags: data.book.pending_tags || [],
        rejected_tags: data.book.rejected_tags || [],
        ai_metadata: data.book.ai_metadata || null,
        image_analysis_data: data.book.image_analysis_data || null
      };
