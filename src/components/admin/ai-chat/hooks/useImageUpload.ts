...
export function useImageUpload(language: UILanguage = 'en') {
  ...
  async function upload(file: File): Promise<string | null> {
    ...
    try {
      ...
      if ((sign.params as any).transformation) fd.append('transformation', (sign.params as any).transformation)
      if ((sign.params as any).public_id) fd.append('public_id', (sign.params as any).public_id)
      if ((sign.params as any).unique_filename) fd.append('unique_filename', String((sign.params as any).unique_filename))
      if ((sign.params as any).overwrite) fd.append('overwrite', String((sign.params as any).overwrite))
      ...
      try {
        ...
        const msg = typeof e2 === 'object' && e2 && 'message' in e2 ? String((e2 as { message?: unknown }).message) : ERROR_MESSAGES[language].upload_failed
        setError(msg)
        return null
      }
    } finally {
