  ...
  while ((m = urlRe.exec(text)) !== null) {
    ...
    parts.push(
      <a key={parts.length}
        ...
        className="text-ctp-blue underline-offset-4 hover:underline break-words">
        {url}
      </a>
    )
...
function emphasize(text: string): Array<string | React.ReactElement> {
  ...
  while ((m = re.exec(text)) !== null) {
    ...
    segments.push(<strong key={segments.length} className="text-ctp-mauve font-semibold">{m[1]}</strong>)
    last = re.lastIndex
  }
  ...
  if (labelMatch) {
    ...
    return (
      <p key={idx} className="leading-7">
        ...
        <span> — </span>
        {linkify(desc).map((t, i) => typeof t === 'string' ? <React.Fragment key={i}>{emphasize(t)}</React.Fragment> : t)}
      </p>
    )
  }
  return (
    <p key={idx} className="leading-7">
      {linkify(line).map((t, i) => typeof t === 'string' ? <React.Fragment key={i}>{emphasize(t)}</React.Fragment> : t)}
    </p>
  )
}
        ...
        return (
          <div key={bi} className="space-y-1">
            {lines.map((l, li) => {
              ...
              const el = renderLine(l, li)
              if (isLead) {
                firstParaRendered = true
                return React.cloneElement(el, { className: cn(el.props.className, 'text-ctp-mauve font-medium') })
              }
              return el
            })}
          </div>
