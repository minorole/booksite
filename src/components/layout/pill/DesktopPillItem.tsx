...
export function DesktopPillItem({ item, activeHref, ease = "power3.easeOut", index, isOpen, onOpen, onClose }: Props) {
  ...
  useEffect(() => {
    ...
    if (isOpen) {
      ...
      gsap.to(dd, { opacity: 0, y: 6, duration: 0.15, ease, onComplete: () => { gsap.set(dd, { visibility: "hidden" }) } })
    }
  }, [isOpen, ease])
  ...
  useEffect(() => {
    ...
    activeTweenRef.current = tl.tweenTo(isOpen ? tl.duration() : 0, { duration: 0.2, ease, overwrite: "auto" })
  }, [isOpen, ease])
  ...
  const PillContent = (
    <>
      <span ref={circleRef} className="hover-circle absolute left-1/2 bottom-0 rounded-full z-[1] block pointer-events-none" style={{ background: "var(--base, #000)", willChange: "transform" }} aria-hidden="true" />
      {item.custom ? (
        <span className="relative inline-flex items-center z-[2]">{item.custom}</span>
      ) : (
    ...
    <li ref={wrapperRef} role="none" className="flex h-full relative">
      {item.custom && !isLink ? (
        ...
        <>
          ...
          <div
            ref={ddRef}
            className="absolute z-[999] top-[calc(100%+4px)] left-1/2 -translate-x-1/2 rounded-[20px] ring-1 ring-black/10 shadow-[0_8px_32px_rgba(0,0,0,0.16)] overflow-hidden"
            style={{
          ...
          >
            <ul className="list-none m-0 p-[6px] flex flex-col gap-[6px]" role="menu">
              {item.children.map((child, ci) => {
                ...
                const cls = "block py-3 px-4 text-[16px] font-medium rounded-[50px] transition-all duration-200 ease-[cubic-bezier(0.25,0.1,0.25,1)] whitespace-nowrap"
                return (
                  <li key={(child.href ?? "child-") + ci} role="none">
                    {isRouterLink(child.href) ? (
                      <Link role="menuitem" href={child.href} aria-label={child.ariaLabel || (typeof child.label === "string" ? child.label : undefined)} className={cls} style={defaultStyle} onMouseEnter={hoverIn} onMouseLeave={hoverOut} onClick={onClose}>
                        {child.label}
                      </Link>
                    ) : (
                      <a role="menuitem" href={child.href} aria-label={child.ariaLabel || (typeof child.label === "string" ? child.label : undefined)} className={cls} style={defaultStyle} onMouseEnter={hoverIn} onMouseLeave={hoverOut} onClick={onClose}>
                        {child.label}
                      </a>
                    )}
                  </li>
        ...
        <Link role="menuitem" href={item.href!} className={basePillClasses} style={pillStyle} aria-label={item.ariaLabel || (typeof item.label === "string" ? item.label : undefined)} onMouseEnter={handleEnter} onMouseLeave={handleLeave} data-pill>
          {PillContent}
        </Link>
      ) : (
        <a role="menuitem" href={item.href} className={basePillClasses} style={pillStyle} aria-label={item.ariaLabel || (typeof item.label === "string" ? item.label : undefined)} onMouseEnter={handleEnter} onMouseLeave={handleLeave} data-pill>
          {PillContent}
        </a>
      )}
    </li>
