...
export function MobileMenu({
  ...
  const isLanguageItem = (it: PillNavItem) =>
    typeof it.label === 'string' &&
    (it.label === '中文' || it.label === 'English' || it.label === 'ENGLISH');
  const isSignInItem = (it: PillNavItem) =>
    typeof it.href === 'string' && it.href.includes('/auth/signin');
  ...
  const mainItems: PillNavItem[] = items.filter(
    (it) => !isLanguageItem(it) && !isSignInItem(it) && !it.custom,
  );
      ...
      <DialogPrimitive.Root open={isOpen} onOpenChange={handleOpenChange}>
        <DialogPrimitive.Portal>
          ...
          <DialogPrimitive.Overlay
            ref={overlayRef}
            className="fixed inset-0 z-[997] bg-black/40 md:hidden"
          ...
          <div
            ref={preLayersRef}
            className="pointer-events-none fixed inset-y-0 right-0 z-[998] md:hidden"
            style={{ opacity: 1, width: 'min(50vw, 420px)' }}
            aria-hidden="true"
          >
            <div
              className="sm-prelayer absolute inset-y-0 right-0 w-full"
              style={{ background: 'var(--base, #000)', opacity: 0.1 }}
            />
            <div
              className="sm-prelayer absolute inset-y-0 right-0 w-full"
              style={{ background: 'var(--base, #000)', opacity: 0.14 }}
            />
            <div
              className="sm-prelayer absolute inset-y-0 right-0 w-full"
              style={{ background: 'var(--base, #000)', opacity: 0.18 }}
            />
          </div>
          ...
          <DialogPrimitive.Content
            ref={panelRef}
            id={menuId}
            className="fixed inset-y-0 right-0 z-[999] overflow-y-auto overscroll-contain md:hidden"
          ...
          >
            ...
            <div className="relative flex h-full w-full flex-col gap-4 bg-white/95 px-5 py-6 backdrop-blur-md">
              {/* In-panel close button */}
              <button
                type="button"
                aria-label="Close menu"
                onClick={requestClose}
                className="absolute top-3 right-3 inline-flex h-9 w-9 items-center justify-center rounded-full bg-black/10 text-black hover:bg-black/15 focus:outline-none focus-visible:ring-2 focus-visible:ring-black/30"
              >
