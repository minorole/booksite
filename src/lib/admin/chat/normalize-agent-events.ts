...
export function normalizeRunItemToAssistantEvents(input: { name?: string; raw?: unknown }): NormalizedEvent[] {
...
export function normalizeRunItemToSSEEvents(input: { name?: string; raw?: unknown }): NormalizedEvent[] {
  const { name, raw } = input
  if (name === 'tool_called') {
  ...
  if (name === 'tool_output') {
    ...
    if (out?.type === 'function_call_result') {
      ...
      const successFlag = envelope && typeof envelope.success === 'boolean' ? (envelope.success as boolean) : true
      ...
      const contentStr = typeof payload === 'string' ? (payload as string) : JSON.stringify(payload)
      events.push({ type: 'tool_append', message: { role: 'tool', name: out.name as string, tool_call_id: out.callId as string, content: contentStr } })
      return events
    }
    return []
  }
...
export function normalizeAgentUpdatedToSSEEvents(input: { agent?: { name?: string } | null | undefined }): NormalizedEvent[] {
