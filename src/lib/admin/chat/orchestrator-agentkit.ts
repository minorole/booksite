...
import { adminAiLogsEnabled, adminAiSensitiveEnabled, debugLogsEnabled } from '@/lib/observability/toggle'
...
export async function runChatWithAgentsStream(params: {
  ...
  let handoffs = 0
  const report = () => {
    try { params.onMetrics?.({ turns, toolCalls, handoffs }) } catch {}
  }
      ...
      if (e.type === 'run_item_stream_event') {
        ...
        for (const ne of events) {
          ...
          if (ne.type === 'tool_start') {
            ...
            toolCalls += 1
            report()
            const argsBytes = (() => { try { return JSON.stringify(ne.args ?? {}).length } catch { return 0 } })()
            try { toolStartAt.set(ne.id, Date.now()) } catch {}
            if (adminAiLogsEnabled()) {
              try { console.log('[AdminAI orchestrator] tool_start', { req: params.requestId?.slice(0, 8), name: ne.name, id: ne.id, argsBytes }) } catch {}
            }
            try {
            ...
            if (adminAiLogsEnabled()) {
              try { console.log('[AdminAI orchestrator] tool_result', { req: params.requestId?.slice(0, 8), name: ne.name, id: ne.id, success: ne.success, durationMs }) } catch {}
            }
            try {
