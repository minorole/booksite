...
describe('Orchestrator DI engine', () => {
  ...
  it('uses injected engine stream and emits normalized SSE events', async () => {
    const fakeEngine = {
      async run() {
        async function* gen() {
          yield { type: 'run_item_stream_event', name: 'message_delta', item: { rawItem: { type: 'output_text.delta', delta: 'A' } } }
          yield { type: 'run_item_stream_event', name: 'tool_called', item: { rawItem: { type: 'function_call', name: 'check_duplicates', callId: '1', arguments: { title: 't' } } } }
          yield { type: 'run_item_stream_event', name: 'tool_output', item: { rawItem: { type: 'function_call_result', name: 'check_duplicates', callId: '1', output: { type: 'json', json: { success: true, data: { has_duplicates: false } } } } } }
        }
        return gen()
      },
    }
    ...
    for (const e of events) {
      if (['assistant_delta','assistant_done','handoff','tool_start','tool_result','tool_append'].includes(e.type)) {
