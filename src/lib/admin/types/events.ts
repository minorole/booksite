...
export function isSSEEvent(x: unknown): x is SSEEvent {
  ...
  switch (t) {
    ...
    case 'tool_result':
      return isString((x as any).id) && isString((x as any).name) && typeof (x as any).success === 'boolean' && 'result' in x && isString((x as any).finishedAt)
    case 'tool_append':
      return isObj((x as any).message) && (x as any).message.role === 'tool' && isString((x as any).message.content)
    case 'error':
