...
export function useUsers(opts: UseUsersOptions = {}) {
  const initialLimit = Number.isFinite(opts.initialLimit as number)
    ? Math.max(1, Math.min(Number(opts.initialLimit), 200))
    : 50;
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      abortRef.current = ac;
      const { users, total } = await fetchUsersApi({
        q: debouncedQuery,
        limit,
        offset,
        hideSuperAdmin: !!opts.hideSuperAdmin,
        signal: ac.signal,
      });
      setUsers(users);
