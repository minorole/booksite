...
export function visionTools(): Tool<AgentContext>[] {
  const analyze = tool({
    ...
    description: 'Analyze a book cover image (structured, single step). Extract only the necessary fields.',
    strict: true,
    ...
    async execute(input: unknown, context?: RunContext<AgentContext>) {
      if (shouldSkipDuplicateCall('analyze_book_cover', input)) {
        return { success: true, message: 'Skipped duplicate analyze_book_cover call (same image/args in this request).' }
      }
  ...
  const analyzeItem = tool({
    ...
    async execute(input: unknown, context?: RunContext<AgentContext>) {
      if (shouldSkipDuplicateCall('analyze_item_photo', input)) {
        return { success: true, message: 'Skipped duplicate analyze_item_photo call (same image/args in this request).' }
      }
      const email = context?.context?.userEmail || 'admin@unknown'
