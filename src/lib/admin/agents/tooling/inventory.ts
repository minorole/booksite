...
export function inventoryTools(): Tool<AgentContext>[] {
    ...
    async execute(input: unknown, context?: RunContext<AgentContext>) {
      ...
      if (!payload.confirmed) {
        return {
          ...
          error: { code: 'confirmation_required', details: 'create_book' },
        };
      }
      ...
      if (!mapped.cover_image && !mapped.cover_url) {
        return {
          ...
          error: { code: 'validation_error', details: 'missing_cover' },
        };
      }
      ...
      const result = await createBook(
        mapped as unknown as import('@/lib/admin/types').BookCreate,
        email,
      );
      return result;
    },
  ...
  const update = tool({
    ...
    async execute(input: unknown, context?: RunContext<AgentContext>) {
      ...
      if (typeof input === 'object' && input !== null) {
        ...
        if (!obj.confirmed) {
          return {
            success: false,
            message: 'Confirmation required',
            error: { code: 'confirmation_required', details: 'update_book' },
          };
        }
      ...
      const result = await updateBook(
        sanitized as unknown as import('@/lib/admin/types').BookUpdate,
        email,
      );
      return result;
    },
  ...
  const adjust = tool({
    ...
    description:
      'Increase or decrease the quantity of an existing book by a delta. Ensures non-negative results.',
    strict: true,
