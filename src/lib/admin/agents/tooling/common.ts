...
export function buildCheckDuplicatesTool(): Tool<AgentContext> {
  return tool({
    ...
    description:
      'Check for duplicate books or items based on extracted fields and optional cover image. Provide either book fields (title/author/publisher) or item fields (name/type/tags).',
    strict: true,
    parameters: z
      .object({
