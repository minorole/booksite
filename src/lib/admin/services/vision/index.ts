...
export function validateAnalysisResult(x: unknown): boolean {
  ...
  if (
    !((ld as any).script_types as unknown[]).every(
      (s) => typeof s === 'string' && allowedScripts.has(s),
    )
  )
    return false;
  ...
  const checkTEP = (p: any) =>
    isRecord(p) &&
    typeof p.confidence === 'number' &&
    (p.zh === undefined || typeof p.zh === 'string') &&
    (p.en === undefined || typeof p.en === 'string');
  ...
  if (!((ve as any).notable_elements as unknown[]).every((t) => typeof t === 'string'))
    return false;
