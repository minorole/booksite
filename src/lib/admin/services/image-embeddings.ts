...
import { getBooksMissingImageEmbeddingsClip, upsertBookImageEmbeddingClip } from '@/lib/db/admin/image-embeddings'
import { handleOperationError } from './utils'

export async function backfillMissingImageEmbeddingsClip(limit = 100, batchSize = 10): Promise<{ processed: number; created: number; errors: number } | { success: false; message: string; error: any }> {
  try {
    ...
    for (let i = 0; i < rows.length; i += batchSize) {
      ...
      await Promise.all(
        slice.map(async (r) => {
          try {
            if (!r.cover_image) { processed++; return }
