...
export function parsePublicIdFromUrl(url: string): { publicId: string | null; hasTransforms: boolean } {
  try {
    const marker = '/upload/'
...
export async function promoteTempAsset(url: string, opts?: { permanentFolder?: string }): Promise<PromoteResult> {
  ...
  const permanentFolder = (opts?.permanentFolder && opts.permanentFolder.trim()) || CLOUDINARY_CONFIG.FOLDER
