...
export async function getOrderDb(
  id: string,
): Promise<{ order_id: string; status: string; tracking_number?: string | null } | null> {
  const db = await getServerDb();
  ...
  return {
    order_id: data.id,
    status: data.status,
    tracking_number: (data as { tracking_number: string | null }).tracking_number ?? null,
  };
}

// Search orders by status or free-text (id/notes)
export async function searchOrdersDb(args: {
  status?: string | null;
  q?: string | null;
  limit?: number;
}): Promise<Array<{ order_id: string; status: string; tracking_number?: string | null }>> {
  const db = await getServerDb();
