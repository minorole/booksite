...
export async function upsertBookImageEmbeddingClip(
  bookId: string,
  embedding: number[],
): Promise<void> {
...
export async function getBooksMissingImageEmbeddingsClip(
  limit = 100,
): Promise<Array<{ id: string; cover_image: string | null }>> {
  const db = (await getServerDb()) as any;
