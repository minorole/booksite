...
export async function checkDuplicatesDb(args: {
  ...
  let query = db
    .from('books')
    .select(
      'id, title_zh, title_en, description_zh, description_en, cover_image, quantity, category_id',
    );
  ...
  if (args.category_type) {
    filtered = filtered.filter(
      (b) => (catTypeById.get(b.category_id as string) ?? 'OTHER_BOOKS') === args.category_type,
    );
  }
