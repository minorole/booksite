...
export async function POST(request: Request) {
  try {
    ...
    const daysInput = Number.isFinite(Number(daysParam))
      ? Number(daysParam)
    ...
    do {
      ...
      const batch = (res.resources || []) as Array<{
        public_id: string;
        created_at: string;
        secure_url?: string;
      }>;
      ...
      for (const r of oldOnes) {
        ...
        if (!dry) {
          await withRetry(
            () => cloudinary.api.delete_resources([r.public_id], { invalidate: true }),
            'delete resource',
          );
          deleted++;
          await sleep(50);
    ...
    return NextResponse.json(
      { success: false, error: 'Failed to purge temp uploads' },
      { status: 500 },
    );
  }
}
