...
export async function PUT(request: Request, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    try {
      ...
      console.log('[admin] update_book', {
        id,
        keys,
        tagsCount: Array.isArray((data as any)?.tags) ? (data as any).tags.length : undefined,
      });
    } catch {}
...
export async function DELETE(request: Request, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    const { error: delErr } = await dbmod.from('books').delete().eq('id', id);
    if (delErr) {
