...
export async function POST(request: Request) {
  try {
    ...
    const signature = request.headers.get('x-cld-signature') || request.headers.get('X-Cld-Signature') || undefined
    const tsHeader = request.headers.get('x-cld-timestamp') || request.headers.get('X-Cld-Timestamp') || undefined
