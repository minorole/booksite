...
Decision
1) Move tool orchestration to the server and keep the client thin. 2) Refactor the large chat UI into focused components and hooks. 3) Add a streaming route for text chat. 4) Default OpenAI model to `gpt-5-mini` for both text and vision. 5) Centralize UI i18n messages. 6) Add an opt‑in path to the Responses API while preserving the existing public helpers.
...
- Streaming (text UX)
  - New `src/app/api/admin/ai-chat/stream/route.ts` that streams assistant content via SSE using the same OpenAI wrapper utilities; it does not execute tools (content‑only path for responsiveness).
...
- OpenAI model defaults and Responses API (opt‑in)
  - `src/lib/openai.ts`:
    ...
    - Added an opt‑in path for the Responses API (`OPENAI_USE_RESPONSES=1`) for safe, no‑tools, non‑streaming requests; wraps outputs into a Chat Completions‑shaped object so callers remain unchanged.
...
- `/api/admin/ai-chat/stream` is a new route for assistant text streaming only (no tool execution).
...
- Responses API everywhere: deferred. We added an opt‑in that covers safe cases, but full parity for tools/streaming across Responses vs Chat Completions should be validated separately.
