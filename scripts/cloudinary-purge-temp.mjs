...
async function main() {
  ...
  async function withRetry(fn, desc, retries = 3, baseMs = 500) {
    ...
    while (true) {
      try {
        ...
        console.warn(`[purge] ${desc} failed (attempt ${attempt}/${retries}), retrying in ${delay}ms...`, e?.message || e)
        await sleep(delay)
      }
    }
  }
  ...
  let eligible = 0
  let scanned = 0
  console.log(`[purge] ${dryRun ? 'DRY-RUN: would delete' : 'Deleting'} assets tagged 'temp' or in ${prefix} older than ${before.toISOString()}`)
  do {
    ...
    if (!dryRun && oldOnes.length > 0) {
      ...
      for (let i = 0; i < ids.length; i += 100) {
        ...
        await withRetry(() => cloudinary.api.delete_resources(chunk, { invalidate: true }), 'delete resources')
  ...
  console.log(`[purge] Done. Scanned: ${scanned}, Eligible: ${eligible}, Deleted: ${deleted}, Days: ${days}, Prefix: ${prefix}${dryRun ? ', DRY-RUN' : ''}`)
}
